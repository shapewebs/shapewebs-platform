...
export type PortalTurnstileAction =
  "customer_invitation" | "customer_registration" | "customer_recovery";
...
export async function verifyPortalTurnstile(input: {
  action: PortalTurnstileAction;
  ip?: string | null;
  token: string | null;
}): Promise<boolean> {
  const expectedHostname = process.env.PORTAL_TURNSTILE_EXPECTED_HOSTNAME;
  const secret = process.env.PORTAL_TURNSTILE_SECRET_KEY;
  const siteKey = process.env.NEXT_PUBLIC_PORTAL_TURNSTILE_SITE_KEY;
  ...
  const testMode = process.env.PORTAL_TURNSTILE_TEST_MODE === "true";
  if (
    testMode &&
...
export function getPortalRequestIp(request: Request): string | null {
