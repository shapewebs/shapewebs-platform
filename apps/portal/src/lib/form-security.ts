...
import { getPortalBaseUrl, isPortalRuntimeReady } from "./auth-environment";
...
function getFormSecret(): string | null {
  ...
  return isPortalRuntimeReady() && secret && secret.length >= 32
    ? secret
    : null;
}
...
export function createPortalFormToken(
  ...
  if (!secret) {
    throw new Error("Customer portal form security is unavailable.");
  }
...
function verifyPortalFormToken(
  token: string,
  action: PortalFormAction,
  now = Date.now(),
): boolean {
...
export async function readSecurePortalForm(
  request: Request,
  action: PortalFormAction,
): Promise<
...
> {
  const expectedOrigin = getPortalBaseUrl();
  if (!expectedOrigin) {
  ...
  const csrfToken = getSingleFormValue(params, "csrfToken", 512);

  return csrfToken && verifyPortalFormToken(csrfToken, action)
    ? { params, status: "ok" }
    : { status: "invalid" };
}
...
export function portalRedirectResponse(
  request: Request,
  pathname: string,
  setCookies: string[] = [],
): Response {
...
export function portalFormErrorResponse(
  request: Request,
  pathname: string,
  status: "invalid" | "too_large" | "unavailable",
): Response {
