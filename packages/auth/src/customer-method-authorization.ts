...
const methodAuthorizationLifetimeMs = 60 * 1_000;

type CustomerMethodAuthorization = {
  action: "link_google";
  issuedAt: number;
  nonce: string;
  sessionId: string;
...
export function createCustomerMethodAuthorization(
  ...
  if (
    ...
    throw new Error("Customer method authorization input is invalid.");
  }

  const payload: CustomerMethodAuthorization = {
    ...input,
    issuedAt: now,
...
export function verifyCustomerMethodAuthorization(
  ...
  try {
    ...
    ) as Partial<CustomerMethodAuthorization>;
    ...
    return (
      ...
      payload.issuedAt >= now - methodAuthorizationLifetimeMs &&
