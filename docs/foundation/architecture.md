...
Build Shapewebs as a small monorepo containing three independently buildable
Next.js applications, with the customer portal deployed only when its identity
and tenant-isolation gates are complete:
...
2. **Admin platform** — `admin.shapewebs.com`: allowlisted Google sign-in,
   mandatory local TOTP step-up, the custom admin CMS, media management, and
   privileged operations. It is dynamic where identity requires it and has a
   separate security and performance budget.
3. **Customer portal** — `portal.shapewebs.com`: invitation-gated customer
   project access with either verified email/password or Google. It has an
   independent Better Auth instance, cookie namespace, OAuth client, database
   schema, runtime role, deployment, and assurance lifecycle. Its repository
   foundation exists, but all UI routes remain code-gated with HTTP `503` until
   customer identity and forced-RLS isolation are proven.
...
only be introduced after a measured need.
...
flowchart LR
    ...
    Customer["Invited customer"] --> Portal["portal.shapewebs.com<br/>Next.js customer portal"]
    Platform --> Auth["Admin Better Auth<br/>Google + TOTP step-up"]
...
- The admin platform needs tightly allowlisted sessions, OAuth callbacks,
  mandatory step-up, draft content, mutation routes, an editor, and privileged
  provider access.
- The customer portal needs invitation, credential and Google account flows,
  tenant/project authorization, and customer-scoped private data without
  inheriting administrative privileges.
...
- Customer identity cannot weaken the admin allowlist or turn the public
  portfolio into an authenticated application.
