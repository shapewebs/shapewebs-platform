...
| Employee and customer password storage                       | Memory-hard password KDF                                              | Pinned Better Auth scrypt implementation                                                                            |
...
| --------------------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `BETTER_AUTH_SECRET`                          | Canonical account cookies, encrypted OAuth tokens and employee TOTP seeds       | Vercel encrypted environment, exact authenticated-app scope                                        | suspected exposure, account compromise, cryptographic migration or scheduled annual exercise |
| `ADMIN_AUTH_EMAIL_ENCRYPTION_SECRET`          | Encrypt invitation, verification, activation and reset tokens awaiting delivery | Authenticated app/outbox worker encrypted environment only                                         | suspected exposure, outbox redesign, environment retirement or annual exercise               |
| Google OAuth client secret                    | Canonical account Google OAuth token exchange                                   | Google Cloud and authenticated-app Vercel environment                                              | suspected exposure, OAuth-client ownership change or annual exercise                         |
| Google OAuth client ID                        | Public OAuth identifier for the canonical account client                        | Google Cloud and authenticated-app Vercel environment                                              | OAuth-client replacement                                                                     |
...
| Employee verification/reset tokens            | One-time employee activation and password recovery                              | CSPRNG-generated; SHA-256 lookup hashes in Neon; email copy in an authenticated encrypted envelope | automatic expiry/consumption, recovery completion or exposure                                |
...
| Sanity draft-reader token                     | Read one exact unpublished revision during private preview                      | Sanity, exact web Preview environment and approved local keychain                                  | exposure, environment retirement, role change or annual exercise                             |
| Sanity content-writer token                   | Create drafts, upload public media, publish and unpublish; admin only           | Sanity, exact admin Preview environment and approved local keychain                                | exposure, environment retirement, role change or annual exercise                             |
| Sanity webhook signing secret                 | Verify raw content-change webhook bodies                                        | Sanity, exact admin Preview environment and approved local keychain                                | webhook replacement, environment retirement or exposure                                      |
| Sanity preview activation/session tokens      | One-time handoff and bounded access to one exact saved draft revision           | CSPRNG-generated; only SHA-256 hashes persist in Neon; session token only in a host-only cookie    | automatic expiry, one-time activation consumption, explicit exit or session revocation       |
