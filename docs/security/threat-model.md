...
password, and passkeys are attachable login methods on that identity; passkeys
...
flowchart LR
  ...
  AccountApp --> StaffAuthorization["Staff membership plus verified assurance"]
...
entry point re-reads active staff membership and server-owned assurance
freshness. Google/password sessions establish that assurance through local
TOTP, while a signed passkey assertion with authenticator user verification
establishes it on the exact newly created session.
...
| Customer-to-admin privilege crossing  | Independent staff authorization plus server-owned strong-auth assurance on every studio entry point; separate least-privilege repositories | Same-account customer-only and suspended-staff denial tests       |
...
| Passkey assurance spoofing     | Browser → Better Auth    | Exact origin/RP/challenge/signature/counter checks, required authenticator user verification, and assurance written only by the exact successful verification route | Wrong origin/RP, replay, missing UV, wrong route/method and direct-session tests |
