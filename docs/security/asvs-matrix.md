...
| -------------------------------------------- | ------ | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| Architecture and trust boundaries            | L2     | Implemented | `docs/security/threat-model.md`, isolated public/authenticated applications and package boundaries                                                                          | Owner        |
| Secure development lifecycle                 | L2     | Implemented | Required CI, roadmap, ADRs, PR template, repository ruleset                                                                                                                 | Owner        |
| Authentication library                       | L2     | Implemented | One canonical Better Auth identity realm in `apps/admin`, Drizzle adapter, multi-method UI and fail-closed environment validation                                           | Owner        |
...
| Employee multi-method authentication         | L2     | Partial     | Allowlisted Google/password plus TOTP, user-verified passkey assurance, durable verified email and explicit session-bound linking are implemented; staging proof is pending | Owner        |
| Customer invitation and credentials          | L2     | Partial     | One-time invitation, mailbox-owned password activation, Google grant, throttles and session limits are implemented in the unified app; staging proof pending                | Owner        |
